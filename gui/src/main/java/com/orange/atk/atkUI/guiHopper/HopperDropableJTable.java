 ...
 * 
 * ------------------------------------------------------------------
 * File Name   : HopperDropableJTable.java
 *
 * Created     : 04/07/2007
 ...
 * @since JDK5.0
 */
public class HopperDropableJTable extends JTable implements DropTargetListener {
	...
	public void drop(DropTargetDropEvent event) {
		...
		try {
			...
			if (dd_flashPathVect.size()>0) {
				//String selectedProfile = AutomaticPhoneDetection.getDeviceType();
				...
				while (itSWF.hasNext()) {
					...
					HopperStep flashStep = new HopperStep(swfPath);
