 ...
 * 
 * ------------------------------------------------------------------
 * File Name   : MixScriptPropertiesDialog.java
 *
 * Created     : 28/05/2007
 ...
 * @since JDK5.0
 */
public class MixScriptPropertiesDialog extends MixScriptStepDialog {
	 ...
	 * @param MixScriptStep
	 */
	public MixScriptPropertiesDialog(MixScriptStep MixScriptStep) {
		super();
		initializeFields(MixScriptStep);
		this.setTitle("Step properties editor");
		this.setVisible(true);
	}

	protected void initializeFields(MixScriptStep jatkStep) {
	...
	protected void action() {
		...
		if (!fileError) {
			...
			if (clt.getTable().getSelectedRowCount() == 1) {
				...
				MixScriptStep cmdLine = (MixScriptStep)(clt.getCampaign().get(numCampaign));
				// authentication
				...
				if (nameIsChanged(flashURIOld, flashURI)) {
					...
					model.setValueAt(flashName, indexRow, MixScriptCheckListTable.COLUMN_TESTNAME);
					String verdict = (String)model.getValueAt(indexRow, MixScriptCheckListTable.COLUMN_VERDICT);
					if (verdict != null && !verdict.equals("")) {
						CoreGUIPlugin.mainFrame.setModified(true);
						model.setValueAt("M ", indexRow, MixScriptCheckListTable.COLUMN_MODIFIED);
					}
