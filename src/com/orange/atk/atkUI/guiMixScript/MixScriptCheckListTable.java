...
public class MixScriptCheckListTable extends CheckListTable {
	...
	private Object[] longValues = {"1000", "ALongWordForAFlashFile.swf", "*", "Passed***",
			"A long comment"};
	...
	private JPopupMenu createTablePopUp() {
		...
		popup.add(MixScriptGUIAction.SETSCREENSHOTREFERENCEDIR
		...
		public Component getTableCellRendererComponent(JTable table, Object color,
			...
			try {
				...
				if (column == COLUMN_SCREENSHOT) {
					...
					//
					this.setIcon(new ImageIcon(CoreGUIPlugin.getIconURL("tango/camera_icon.jpg"),
				...
				if (column == COLUMN_ANALYSER) {
					this.setIcon(new ImageIcon(CoreGUIPlugin.getIconURL("tango/analyser.jpg"),
							"Launch Analyser"));
				}
				if (completeView) {
					if (column == COLUMN_COMMENTS) {
	...
	public void addRow(Step step, int rowNumberInGUI, boolean selectIt, boolean checkPreviousResults) {
		...
		if (checkPreviousResults) {
			...
			// List<MixScriptStepAnalysisResult> results = new
			// ArrayList<MixScriptStepAnalysisResult>();
			...
			// MixScriptStepAnalysisResult rs =
			// (MixScriptStepAnalysisResult)arManager.getPreviousAnalysisResult(flashStep);
			// if (rs!=null) {
			// results.add(rs);
			// }
			// }
	...
	public void updateStep(Step step) {
		 ...
		 * MixScriptStepAnalysisResult rs =
		 * (MixScriptStepAnalysisResult)arManager
		 * .getPreviousAnalysisResult(flashStep); if (rs!=null) {
		 * results.add(rs); } }
		 */
		// looking at retreived results
		if (results.size() == 1) {
