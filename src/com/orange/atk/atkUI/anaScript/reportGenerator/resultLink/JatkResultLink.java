 ...
 * @since JDK5.0
 */
public class JatkResultLink extends ResultLink{

	private Hashtable<String, Vector<JatkResult>> resultsTable;

	/**
	 * @param resultsFile
	 */
	public JatkResultLink(File resultsFile) {
		resultsTable = new Hashtable<String, Vector<JatkResult>>();
	...
	public Vector<? extends Result> getResults(String ruleName) {
		...
		if (resultsTable.containsKey(ruleName)) {
			...
			while (it.hasNext()) {
				...
				JatkResult result = new JatkResult(ruleName);
	...
	public boolean isEmptyResults() {
		Enumeration<Vector<JatkResult>> en = resultsTable.elements();
		while (en.hasMoreElements()) {
			Vector<JatkResult> res = en.nextElement();
