...
/**
 * This class allow to retrieve a file by its path,
 * whatever it has to be downloaded or not.
 * It handles a cache of downloaded files to prevent
...
public class FileResolver {
	...
	/**
	 * A map used to prevent downloading twice the same file (same URI).
	 * Since it is static, it is shared among all FileResolvers
	 * (provided it is loaded by the same classloader!)
	 * */
	private static HashMap<String, File> downloadedFiles = new HashMap<String, File>();

	/**
	 * Build a new FileResolver with the given options
	 * @param tempDir the temporary directory to store downloaded files.
	 * @param httpMaxConTime the maximum time to wait for connexion (in ms)
	 * @param httpMaxDwnTime the maximum time to wait for download (in ms)
	 * @param proxySet wether the proxy have to be set or net
	 * @param proxyHost the name of the proxy
	 * @param ProxyPort the proxy port to use
	 */
	public FileResolver(File tempDir, int httpMaxConTime, int httpMaxDwnTime, int httpMaxAttempts, boolean proxySet, String proxyHost, String ProxyPort) {
	...
	public File getFile(String uri, String prefix, String ext, String login, String password, String user_agent, File tempDir) {
		File file = null;
		if (uri.startsWith("http:")) { // distant file
			...
			if (file==null) {
				...
				Out.log.println("downloading file at '"+uri+"' to '"+file.getAbsolutePath()+"'" );
				file = httpDownload(uri, tempDir, baseFileName, ext, login, password, user_agent);
				Out.log.println("download ok to '"+file.getAbsolutePath()+"'");
				// add it in cache
	...
	/**
	 * Retrieves a file located at the given URI.
	 * It the file is distant, it will be downloaded and put in cache.
	 * @param uri location of the file to retrieve.
	 * @param fileName the name to give to the downloaded file (may be null).
	 * @param login login to use to download the file.
	 * @param password to use to download the file.
	 * @param user_agent to use to download the file.
	 * @return the file or null if there is no file at the given uri.
	 * @throws Alert if the file cannot be found.
	 */
	public File getFile(String uri, String prefix, String ext, String login, String password, String user_agent) {
		return getFile(uri, prefix, ext, login, password, user_agent, tempDir);
	}

	/**
	 * Download the file located at the given URL, and write it to
	 * the given target local directory. Return a File object on
	 * that file.
	 * Exit if there is any problem while downloading, or to store the
	 * downloaded file.
	 * @param location the URL of the file to fetch.
	 * @param targetDir the directory where to save the downloaded file.
	 * @param baseName the base name used to build the file name.
	 * @param ext the extention of the file name
	 * @param login login to use to download the file.
	 * @param password to use to download the file.
	 * @param user_agent to use to download the file.
	 * @return the downloaded file.
	 * @throws Alert if a problem occurs during download.
	 */
	private File httpDownload( String location, File targetDir,
			                   String baseName, String ext,
			                   String login, String password, String user_agent)
	throws Alert {
		...
		while (!done && (attemptsDone<httpMaxAttempts)) {
			...
			downloader = new Downloader(location, target, httpConTimeOut/*httpMaxConTime*/, httpDwnTime/*httpMaxDwnTime*/, downloadMonitor);
			...
			if ((user_agent!=null)&&(user_agent.trim().length()>0)) {
				downloader.setUser_agent(user_agent);
			}
			downloader.start();
			...
			if (status==DownloadMonitor.DLOAD_SUCCESS) {
				...
				if (status==DownloadMonitor.DLOAD_TIMEOUT_C) {
					...
					Out.log.println("Connexion timeout increased");
					attemptsDone++;
				} else if (status==DownloadMonitor.DLOAD_TIMEOUT_T) {
					httpDwnTime = httpDwnTime*2;
					Out.log.println("Download timeout increased");
					attemptsDone++;
