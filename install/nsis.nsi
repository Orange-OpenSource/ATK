...
loop:
    ...
    DetailPrint $R0
    DetailPrint $R1
    ${ConfigWrite} "$INSTDIR\Config.properties" $3 $R0 $3
    DetailPrint $3
    goto loop
endloop:
...
nsisunz::UnzipToLog "$TEMP\android.zip" "c:\android"
...
; copy all files except the files preceded by /x
    ...
    CreateShortcut $SMPROGRAMS\$StartMenuGroup\ScreenShotComparator.lnk $INSTDIR\ScreenshotComparator.exe "" $INSTDIR\ScreenshotComparator.exe 0
...
Function CustomGUIInit
    ...
    File /oname=$PLUGINSDIR\bgimage.bmp img\logo_rgb.bmp
